...
class ReplaceByFeeViewController : UIViewController, Subscriber, ModalPresentable, Trackable {
    ...
    init(store: Store, sender: Sender, walletManager: WalletManager, originalTx: BRTxRef, initialAmount: UInt64, initialAddress: String? = nil, initialRequest: PaymentRequest? = nil) {
        self.store = store
        self.sender = sender
        self.walletManager = walletManager
        self.initialAmount = initialAmount
        self.originalTx = originalTx
    ...
    private let originalTx: BRTxRef
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if initialAmount != 0 {
            amountView.forceUpdateAmount(amount: Satoshis(initialAmount ))
        }
        sendTapped()
    }

    private func addButtonActions() {
        /*addressCell.paste.addTarget(self, action: #selector(ReplaceByFeeViewController.pasteTapped), for: .touchUpInside)
        addressCell.scan.addTarget(self, action: #selector(ReplaceByFeeViewController.scanTapped), for: .touchUpInside)
       ...
       /* amountView.didChangeFirstResponder = { [weak self] isFirstResponder in
        ...
        amountView.advancedButton.addTarget(self, action: #selector(ReplaceByFeeViewController.advancedTapped), for: .touchUpInside)*/
    }
    ...
    @objc private func sendTapped() {
        ...
        if sender.transaction == nil {
            ...
            guard sender.createTransactionReplaceByFee(amount: amount.rawValue, to: address, txn: originalTx) else {
...
extension ReplaceByFeeViewController : ModalDisplayable {
